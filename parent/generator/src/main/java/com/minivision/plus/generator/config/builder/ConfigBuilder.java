...
import com.minivision.plus.generator.config.*;
import com.minivision.plus.generator.config.po.TableField;
import com.minivision.plus.generator.config.po.TableFill;
import com.minivision.plus.generator.config.po.TableInfo;
import com.minivision.plus.generator.config.querys.H2Query;
...
public class ConfigBuilder {
    ...
    private void checkImportPackages(TableInfo tableInfo) {
        if (StringUtils.isNotEmpty(strategyConfig.getSuperEntityClass())) {
            ...
            // 无父类开启 AR 模式
            tableInfo.getImportPackages().add(com.minivision.plus.extension.activerecord.Model.class.getCanonicalName());
        }
        if (null != globalConfig.getIdType()) {
            // 指定需要 IdType 场景
            tableInfo.getImportPackages().add(com.minivision.plus.annotation.IdType.class.getCanonicalName());
            tableInfo.getImportPackages().add(com.minivision.plus.annotation.TableId.class.getCanonicalName());
        }
        if (StringUtils.isNotEmpty(strategyConfig.getVersionFieldName())) {
            tableInfo.getFields().forEach(f -> {
                if (strategyConfig.getVersionFieldName().equals(f.getName())) {
                    tableInfo.getImportPackages().add(com.minivision.plus.annotation.Version.class.getCanonicalName());
                }
            });
        }
    }
            ...
            //oracle数据库表太多，出现最大游标错误
                ...
                if (isInclude) {
                    ...
                    Arrays.stream(config.getInclude())
                            .forEach(tbname -> sb.append(StringPool.SINGLE_QUOTE).append(tbname.toUpperCase()).append("',"));
     ...
     * @return 连接后的包名
     */
    private String joinPackage(String parent, String subPackage, String moduleName) {
